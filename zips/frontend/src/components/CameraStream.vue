...
export default {
  ...
  methods: {
    ...
    async startCamera() {
      ...
      try {
        // Eğer streamUrl varsa onu kullan, yoksa getUserMedia kullan
        if (this.streamUrl) {
          this.$refs.videoElement.src = this.streamUrl
          this.onVideoLoaded()
        } else {
          // Kullanıcı kamerasını kullan
        ...
        this.statusMessage = 'Kamera erişim izni gerekli veya kamera bulunamadı'
