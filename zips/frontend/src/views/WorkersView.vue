...
export default {
  ...
  methods: {
    ...
    async loadManagers() {
      try {
        const response = await fetch('http://localhost:8000/workers?role=manager')
        if (response.ok) {
    ...
    async loadStatistics() {
      try {
        const response = await fetch('http://localhost:8000/workers/statistics')
        if (response.ok) {
    ...
    async saveWorker() {
      ...
      try {
        ...
        if (this.editingWorker) {
          response = await fetch(`http://localhost:8000/workers/${this.editingWorker.id}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(workerData)
          })
        } else {
          response = await fetch('http://localhost:8000/workers', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(workerData)
          })
    ...
    async loadWorkerHistory() {
      ...
      try {
        ...
        const response = await fetch(`http://localhost:8000/workers/history?${params}`)
        if (response.ok) {
    ...
    async savePPEAssignment() {
      ...
      try {
        ...
        const response = await fetch('http://localhost:8000/workers/assign-ppe', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
    ...
    async removePPE(worker, ppe) {
      try {
        const response = await fetch(`http://localhost:8000/workers/${worker.id}/ppe/${ppe.type}`, {
          method: 'DELETE'
        })
    ...
    async processImportFile(file) {
      try {
        ...
        const response = await fetch('http://localhost:8000/workers/import/preview', {
          method: 'POST',
          body: formData
        })
    ...
    async downloadTemplate() {
      try {
        const response = await fetch('http://localhost:8000/workers/import/template')
        if (response.ok) {
          const blob = await response.blob()
          const url = window.URL.createObjectURL(blob)
          const link = document.createElement('a')
    ...
    async startImport() {
      ...
      try {
        const response = await fetch('http://localhost:8000/workers/import', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
    ...
    async generateReport(worker) {
      try {
        const response = await fetch(`http://localhost:8000/workers/${worker.id}/report`, {
          method: 'POST'
        })
    ...
    async deactivateWorker(worker) {
      if (confirm(`${worker.name} çalışanını pasifleştirmek istediğinizden emin misiniz?`)) {
        try {
          const response = await fetch(`http://localhost:8000/workers/${worker.id}/deactivate`, {
            method: 'PUT'
          })
